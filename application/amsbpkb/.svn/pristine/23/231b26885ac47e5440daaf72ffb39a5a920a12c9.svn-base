<?php
	...
	function get_decrypted($encrypt) {
		$decrypted = "";
		$password = '3sc3RLrpd17';
		$method = 'aes-256-cbc'; 
		$key = substr(hash('sha256', $password, true), 0, 32);
		$iv = chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0) . chr(0x0);
		if ($encrypt !=""){
			$decrypted = openssl_decrypt(base64_decode($encrypt), $method, $key, OPENSSL_RAW_DATA, $iv);
	...
	if (!function_exists('encrypString')){
		function encrypString($string) {
			$password = '3sc3RLrpd17';
			$method = 'aes-256-cbc';
			$encrypted = "";
			if ($string != "") {
	...
	if (!function_exists('decryptString')){
		function decryptString($string) {
			...
			if ($string != "") {
				...
				// av3DYGLkwBsErphcyYp+imUW4QKs19hUnFyyYcXwURU=
				// $encrypted = base64_encode(openssl_encrypt($string, $method, $password, OPENSSL_RAW_DATA, $iv));
				// $encrypted="t8NQTKWy7VPag41zQmdVosUCkcRSLtW4PTKtFz+RkRqhqRXcWd1gNPjMybKaFBUQ5Fqhpha08yD8Pp6lcVOigxbgqfAwomZwIUwXtFt4rPM=";
